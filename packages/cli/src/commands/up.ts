...
import { startDesktopGateway } from '../gateway/desktop.js';
...
export interface UpOptions {
  ...
  /** Host HTTPS on this computer, without an external relay or connector. */
  desktop?: DesktopGateway;
  startDesktopImpl?: typeof startDesktopGateway;
...
export async function runUp(opts: UpOptions = {}): Promise<UpHandle> {
  ...
  const desktop = !noTunnel && opts.desktop ? { hostname: desktopHostname(opts.desktop.hostname) } : null;
  const relay = noTunnel || desktop ? null : await readRelayConfig(resolve(cwd, opts.relay ?? '.airlock/relay.json'));
  const publishing = !!(relay || desktop);
  const instance = randomUUID();
  if (desktop && !(opts.access?.operator || process.env.AIRLOCK_OPERATOR_TOKEN)) {
    throw new Error('Desktop publishing requires management access. Start from the Airlock launcher to set it up automatically.');
  }
  ...
  if (opts.docker) {
    ...
    if (publishing) dockerEnv.AIRLOCK_PUBLIC_INSTANCE = instance;
  ...
  if (publishing) spawnEnv.AIRLOCK_PUBLIC_INSTANCE = instance;
  ...
  try {
    ...
    if (publishing) {
      ...
      if (desktop) {
        console.log('  starting your desktop gateway and checking HTTPS…');
        tunnel = await (opts.startDesktopImpl ?? startDesktopGateway)(port, { config: desktop, instance });
      } else if (relay) {
