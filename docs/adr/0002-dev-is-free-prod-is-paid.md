...
The natural funnel for `airlock-deploy` is dev → prod: solo devs and OSS authors build agents on the dev tunnel; when they ship to customers, they graduate to the production path. We chose this as the primary paid wedge instead of an ngrok-style "free tier with restricted tunnel, paid tier with better tunnel" model. The Free signed-in tier is **deliberately generous** — stable subdomain, no time caps, 3 concurrent tunnels, full agent-aware observability, all Shape detectors, ~50 GB/mo bandwidth cap. The paid Pro tier unlocks `airlock-deploy deploy` to the publisher's own cloud, custom domains, longer log retention, and multi-agent management. An Anonymous tier (rotating URL, ~30 min auto-close, 1 tunnel) exists only for the marketing-page 5-second wow; every real user signs up.
