...
Run an agent (any harness) behind a **paid, OpenAI-compatible x402 URL** — self-hosted on your hardware or hosted by airlock. The **host + sell** core of the airlock ecosystem. airlock holds no keys and (self-host) stays out of the request path; the model is always the publisher's (ADR-0008).
...
| Tests | cli 66 · agent-runtime 38 · payment-fly 39 · payment-core 36 · server 30 · payment-workers 12 · payment-fly-node 18 — all green | ✅ |
...
1. **Durable self-host URL** `<name>.airlock.dev` — Cloudflare named-tunnel: `server/cloudflare.ts` provisioner, `POST/DELETE /api/self-host/tunnel`, `tunnel.ts` `named(token)`, `up.ts` minted-token fetch. *(operator prereq: airlock CF account + domain + API token)*
2. **airlock-hosted (Fly) prong** — gating spike (app-scoped deploy token → remote builder + Machines-API), `server/fly.ts`, `POST /api/hosted-deploy`, env-aware `exec.ts` spawner, cost guards. *(operator prereq: airlock Fly org token)*
